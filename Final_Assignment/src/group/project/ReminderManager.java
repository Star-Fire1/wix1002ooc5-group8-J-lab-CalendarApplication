...
public class ReminderManager {
    ...
    public void setReminder(ReminderConfig config) {
        ...
        if (config.getRemindDuration() == null || config.getRemindDuration().trim().isEmpty()) {
            config.setRemindDuration("PT30M");
        }
    ...
    /** Optional: “remove” reminder by writing a disabled config (since no delete api) */
    public void disableReminder(int eventId) {
        if (eventId <= 0) return;
        ReminderConfig c = new ReminderConfig();
        c.setEventId(eventId);
        c.setRemindDuration("PT30M");
        c.setEnable(false);
        setReminder(c);
    }
